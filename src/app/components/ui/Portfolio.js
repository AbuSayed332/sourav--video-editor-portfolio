...
import { Play, ExternalLink, X, Loader } from 'lucide-react'
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {currentItems.map((item, index) => {
                    ...
                    return (
                      <Card
                        key={itemId}
                        className="group bg-gray-800 rounded-xl overflow-hidden hover:transform hover:scale-105 transition-all duration-500 cursor-pointer"
                        ...
                        <div className="relative h-48 overflow-hidden">
                          ...
                          {item.featured && (
                            <div className="absolute bottom-4 right-4">
                              <span className="bg-yellow-500/80 text-black px-2 py-1 rounded text-xs font-bold">
                                Featured
                              </span>
                            </div>
                          )}
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl bg-gray-900 rounded-xl overflow-hidden">
            ...
            <div className="aspect-video">
              {(() => {
                ...
                if (youtubeId) {
                  ...
                  return (
                    <video className="w-full h-full" controls autoPlay src={selectedVideo.videoUrl}>
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gray-800">
                      <div className="text-center">
                        {selectedVideo.thumbnail && (
                          <img src={selectedVideo.thumbnail} alt={selectedVideo.title} className="rounded-lg mb-4 max-h-64 mx-auto" />
                        )}
            ...
            <div className="p-6">
              ...
              {selectedVideo.software?.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {selectedVideo.software.map((s, i) => (
                    <span key={i} className="bg-gray-800 text-gray-300 px-3 py-1 rounded-full text-xs">{s}</span>
                  ))}
                </div>
              )}
            </div>
